...
namespace ContactsCoreMVC.Models.Concrete
{
  public class CosmosContactRepository : IContactRepository
  {
    ...
    {
      ItemResponse<ContactTable>  contactResponse=await _container.CreateItemAsync<ContactTable>(contactTable);
    ...
    public async Task<ContactTable> FindContactAsync(string id)
    {
      ...
      FeedIterator<ContactTable> queryResultIterator=_container.GetItemQueryIterator<ContactTable>(queryDefinition);
      List<ContactTable> contactTableList=new List<ContactTable>();
      while(queryResultIterator.HasMoreResults){
        FeedResponse<ContactTable> currentResultSet= await
           queryResultIterator.ReadNextAsync();
    ...
    public async Task<ContactTable> UpdateAsync(ContactTable contactTable)
    {
     ItemResponse<ContactTable> contactResponse=await
      ...
      contactResponse= await _container.ReplaceItemAsync<ContactTable>(contactResult,
      contactResult.Id);
    ...
    {
      ItemResponse<ContactTable> contactTable= await _container.DeleteItemAsync<ContactTable>(id, new PartitionKey(contactName));
    }
  }
}